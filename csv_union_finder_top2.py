...
for i in range(len(frames)):
	...
	if len(top2) < 2:
		top2.append(i)
		top2nrows.append(nrows)
	else:
		if nrows > min(top2nrows):
			top2.remove(top2[np.argmin(top2nrows)])
			top2.append(i)
			top2nrows.remove(min(top2nrows))
			top2nrows.append(nrows)
	print nrows

print "Indices of largest 2:", top2

if len(top2) == 2:
	df = pd.concat([frames[top2[0]], frames[top2[1]]])
else:
	df = frames[top2[0]]
